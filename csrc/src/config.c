...
void config_set_wifi_ssid(char *ssid) { strncpy(config_wifi_ssid, ssid, sizeof(config_wifi_ssid)); }

void config_set_wifi_key(char *key) { strncpy(config_wifi_key, key, sizeof(config_wifi_key)); }
...
void config_set_lwm2m_uri(char *uri) { strncpy(config_lwm2m_uri, uri, sizeof(config_lwm2m_uri)); }

void config_set_lwm2m_psk_id(char *psk_id) { strncpy(config_lwm2m_psk_id, psk_id, sizeof(config_lwm2m_psk_id)); }

void config_set_lwm2m_psk_key(char *psk_key) { strncpy(config_lwm2m_psk_key, psk_key, sizeof(config_lwm2m_psk_key)); }

void config_set_ntp_url(char *url) { strncpy(config_ntp_url, url, sizeof(config_ntp_url)); }

void config_set_sha256(unsigned char *sha256) { memcpy(config_sha256, sha256, sizeof(config_sha256)); }

void config_set_mqtt_uri(char *uri) { strncpy(config_mqtt_uri, uri, sizeof(config_mqtt_uri)); }
...
void config_set_mqtt_psk_id(char *psk_id) { strncpy(config_mqtt_psk_id, psk_id, sizeof(config_mqtt_psk_id)); }

void config_set_mqtt_psk_key(char *psk_key) { strncpy(config_mqtt_psk_key, psk_key, sizeof(config_mqtt_psk_key)); }

void config_set_http_uri(char *uri) { strncpy(config_http_uri, uri, sizeof(config_http_uri)); }

void config_set_config_uri(char *uri) { strncpy(config_config_uri, uri, sizeof(config_config_uri)); }

void config_set_http_sig_uri(char *sig_uri) { strncpy(config_http_sig_uri, sig_uri, sizeof(config_http_sig_uri)); }

void config_set_http_sig_key(char *key) { strncpy(config_http_key, key, sizeof(config_http_key)); }

char *const config_get_wifi_ssid(void) { return (char *)&config_wifi_ssid[0]; }

char *const config_get_wifi_key(void) { return (char *)&config_wifi_key[0]; }

char *const config_get_lwm2m_uri(void) { return (char *)&config_lwm2m_uri[0]; }

char *const config_get_lwm2m_psk_id(void) { return (char *)&config_lwm2m_psk_id[0]; }

char *const config_get_lwm2m_psk_key(void) { return (char *)&config_lwm2m_psk_key[0]; }

char *const config_get_mqtt_url(void) { return (char *)&config_mqtt_uri[0]; }

char *const config_get_mqtt_device_id(void) { return (char *)&config_mqtt_device_id[0]; }

char *const config_get_lwm2m_endpoint(void) { return (char *)&config_lwm2m_endpoint[0]; }

char *const config_get_mqtt_psk_id(void) { return (char *)&config_mqtt_psk_id[0]; }

char *const config_get_mqtt_psk_key(void) { return (char *)&config_mqtt_psk_key[0]; }

char *const config_get_http_uri(void) { return (char *)&config_http_uri[0]; }

char *const config_get_http_sig_uri(void) { return (char *)&config_http_sig_uri[0]; }

char *const config_get_http_sig_key(void) { return (char *)&config_http_key[0]; }

char *const config_get_config_uri(void) { return (char *)&config_config_uri[0]; }
...
void miso_load_config(void)
{
    ...
    {
        int parse_result = 0;
        parse_result     = jsmn_parse(&parser, (const char *)read_buffer, (size_t)fRead, NULL, 0);
        if (parse_result > 0)
        {
        ...
        parse_result       = jsmn_parse(&parser, (const char *)read_buffer, (size_t)fRead, json_tokens, parse_result);
        ...
        for (int i = 0; i < parse_result; i++)
        {
            ...
            else if (((lwm2m_key + 1) == json_tokens[i].parent) && (json_tokens[i].type == JSMN_STRING))
            {
                ...
                else if ((0 == strncmp(json_key, "endpoint", strlen("endpoint"))) ||
                         (0 == strncmp(json_key, "device", strlen("device"))))
                {
            ...
            else if (((http_key + 1) == json_tokens[i].parent) && (json_tokens[i].type == JSMN_STRING))
            {
                ...
                if ((0 == strncmp(json_key, "sig", strlen("sig"))) ||
                    (0 == strncmp(json_key, "sig_uri", strlen("sig_uri"))))
                {
