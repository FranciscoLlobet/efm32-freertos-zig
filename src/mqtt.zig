...
pub const connection = network.connection;

// Setting alias for types
const mqtt_client = c.mqtt_client;
const mqtt_fd_handle = c.mqtt_pal_socket_handle;
const printf = c.printf;

const network_send = c.uiso_network_send;
const network_read = c.uiso_network_read;

pub export fn mqtt_pal_sendall(fd: ?*anyopaque, buf: [*c]u8, len: usize, flags: i32) callconv(.C) isize {
    _ = flags;
    return @ptrCast(*network.connection, @alignCast(@alignOf(*network.connection), fd)).send(buf, len);
}

pub export fn mqtt_pal_recvall(fd: ?*anyopaque, buf: [*c]u8, len: usize, flags: i32) callconv(.C) isize {
    _ = flags;
    return @ptrCast(*network.connection, @alignCast(@alignOf(*network.connection), fd)).recieve(buf, len);
}
...
pub const mqtt = struct {
    ...
    pub fn publish(self: *@This(), topic: [*:0]const u8, message: [*:0]const u8, message_size: usize, qos: publish_qos, retain: bool) void {
        ...
        if (retain) {
            flags |= @intCast(u8, c.MQTT_PUBLISH_RETAIN);
        }
    ...
    fn initMqtt(self: *@This()) void {
        _ = c.mqtt_init(&self.client, &self.connection, @ptrCast([*c]u8, &send_buf[0]), send_buf.len, @ptrCast([*c]u8, &recv_buf[0]), recv_buf.len, publishResponseCallback);
    }

    fn timer_update_function(xTimer: freertos.TimerHandle_t) callconv(.C) void {
        _ = @as(*@This(), @ptrCast(*@This(), @alignCast(8, freertos.c.pvTimerGetTimerID(xTimer)))).task.notify((1 << 2), .eSetBits);
    }

    fn task_function(pvParameters: ?*anyopaque) callconv(.C) void {
        var self: *@This() = @ptrCast(*@This(), @alignCast(8, pvParameters));
