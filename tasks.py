...
IMG_TOOL = Path.cwd().joinpath("csrc").joinpath("mcuboot").joinpath("mcuboot").joinpath("scripts").joinpath("imgtool.py")
...
def sig_fw_images(c):
    c.run(f"python {IMG_TOOL} sign -v \"0.1.2\" -F 0x40000 -R 0xff --header-size 0x80 --pad-header -k {PRIV_KEY} --overwrite-only --public-key-format full -S 0xB0000 --align 4 {LWM2M_BIN} {LWM2M_SIG_BIN}")
